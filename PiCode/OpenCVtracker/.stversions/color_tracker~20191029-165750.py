...
class ColorTracker:
    ...
    def update(self):
        ...
        while not self.stopped:
            if not self.q.empty():
                time1 = time.time()
                frame_object = self.q.get()
                self.currentFrame = copy.deepcopy(self.currentFrame)
                ...
                if not self.xyDoneQueue.full():
                    ...
                    print(
                        "CT2: {:.2f} Put frame {} to xyDoneQueue (xyDoneQueue size: {})".format((time.time() * 1000),
                                                                                                  self.currentFrame.name,
                                                                                                  self.xyDoneQueue.qsize()), flush=True)
